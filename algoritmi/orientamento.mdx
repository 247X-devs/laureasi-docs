...
flowchart LR
    ...
    Q -->|"Provincia"| C11["Sede in provincia\n+8 / 0"]
    C1 & C2 & C4 & C5 & C6 & C7 & C8 & C9 & C10 & C11 --> TOTAL["Punteggio finale 0-100"]
```
...
| 10 | Sede in provincia | Passo anagrafica | +8 / 0 | Ateneo con almeno una sede nella provincia del lead |
...
Se l’utente seleziona una o più fasce (escl. “Nessuna di queste”), il prezzo nei risultati usa la **retta agevolata** del TaxBreak le cui **Categorie quiz** (`quizCategories`) intersecano le fasce dichiarate (label FE → chiavi canoniche). Ambito ammessi: `university`, `course_type`, `audience_category`, `specific_item` (non `other`). L’API espone `appliedBenefitLabel` e `standardFee`: in UI si mostra il prezzo agevolato, la retta standard barrata e una pill “Agevolazione · …”. Se nessun TaxBreak matcha, resta il prezzo catalogo (`reduced_fee` / standard). Dettagli operativi CMS in [Agevolazioni](/admin/agevolazioni#collegamento-al-quiz-di-orientamento).
...
*Versione 1.6 — Settembre 2026 — Uso interno riservato*
