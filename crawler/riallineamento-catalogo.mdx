...
- **eCampus** — catalogo Cineca `/api/v1/corsi?anno=YYYY`. Dal 2026 la risposta è un albero (`subgroups[].cds[].cdsSub[].cod`), non una lista piatta `{cds}`: `catalog:diff` cammina entrambi. La rinomina riscrive solo `/corsi/YYYY/` e lascia il resto del path (non forza `/insegnamenti` sui master). Non usare lo status HTTP delle pagine corso: la SPA risponde 200 anche se il `cds` non esiste. L'SSD sta sul dettaglio insegnamento (`/api/v1/insegnamento`). Se Cineca ha solo il percorso `comune`, lo script inserisce un piano **Statutario**. Per riempire SSD sui piani già in DB: `node scripts/ecampus-extract-paths.mjs --anno=2026 --enrich-ssd --dry-run`. Per lauree senza percorsi: `--extract-empty`.
...
- **eCampus**: SPA sempre-200; duplicati sullo stesso `cds` e stesso `entity_type` → una rinomina, il resto disattivato. Laurea e `piano_di_studi` sullo stesso CDS si rinomina entrambi. Se il parser Cineca non legge `cdsSub`, il diff vede 0 corsi e disattiva tutto. Per l'SSD usa `corso_percorso_id` dell'attività come `af_percorso`. Piano solo-`comune` → un percorso Statutario, non skip.
