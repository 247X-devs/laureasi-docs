...
- **eCampus** — catalogo Cineca ` /api/v1/corsi?anno=YYYY `. Non usare lo status HTTP delle pagine corso: la SPA risponde 200 anche se il `cds` non esiste.
- **Anno nell'URL** — riscrittura + probe. Se il nuovo URL redirige al vecchio, l'anno non è pubblicato (oggi IUL).
...
- **eCampus**: SPA sempre-200; duplicati sullo stesso `cds` → una rinomina, il resto disattivato.
- **IUL**: `2026-2027` redirige a `2025-2026` finché il sito non pubblica l'anno nuovo.
